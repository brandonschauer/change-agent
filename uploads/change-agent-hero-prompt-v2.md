...
- Single centered column — headline stacked above the illustration + demo card row
- Below the headline: a two-column row at desktop (992px and above) — illustration left, demo card right. Below 992px: illustration hidden, demo card full width centered.

---

### Headline (centered, full width)

- Barlow Semi Condensed 700, Display scale, center-aligned
...
### Illustration (left column, desktop only)

- Image: hero_desk_woman_illustration.png
- No background — the PNG has transparency, place directly on Paper
- Bottom-aligned to the demo card, so her desk appears to sit at the same ground level
- Width: ~40% of the content column
- Hidden below 992px

---

### Demo Card (right column, ~60% width on desktop, full width on mobile)

- White background, border-radius 12px, 1px border in #E0DED8, generous padding
- No header/title bar — conversation area begins immediately inside the card
- Fixed height on load — do not let the card grow vertically during the typing animation. Set a fixed min-height large enough to contain the fully-typed initial exchange, so the card size is stable from the first frame.
...
- The user message pill is rendered at its full final size immediately — only the text inside animates. The card does not resize during typing.
...
- Below 992px: illustration hidden, demo card full width centered, headline remains centered
